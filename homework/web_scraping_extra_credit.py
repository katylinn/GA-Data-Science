...
for li in b.find_all(name='li'):
    print li.text

# print the text of only the API resources
for li in b.find(name='ul', attrs={'id':'api'}).find_all(name='li'):
    print li.text
...
int(b.find(name="time", attrs={"itemprop":"duration"}).text.strip().split(" ")[0])
...
def get_movie_info(imdb_id):
    ...
    dict["duration"] = int(b.find(name="time", attrs={"itemprop":"duration"}).text.strip().split(" ")[0])
    dict["star_rating"] = float(b.find(name="span", attrs={"itemprop":"ratingValue"}).text)
    dict["title"] = b.find(name="h1").find(name="span", attrs={"itemprop":"name"}).text
...
get_movie_info("tt0133093")

# open the file of IDs (one ID per row), and store the IDs in a list

ids = []

with open("/Users/briciola/dev/GA/GA-SEA-DAT1/data/imdb_ids.txt", "r") as f:
...
'''
import codecs
f = codecs.open(r'/Users/briciola/dev/GA/GA-SEA-DAT1/data/Shawshank.html', "w", 'utf-8')
f.write (b.prettify()) 
f.close()
